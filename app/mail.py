...
import sys
sys.path.insert(1,'venv/lib/python3.10/site-packages')
import sendgrid
...
def create():
    if request.method == 'POST':
        ...
        if not email:
            errors.append('Email is necessary')
        if not subject:
            errors.append('Subject is necessary')
        if not content:
            errors.append('A content is necessary')
